...
public partial class Web_Createclient : System.Web.UI.Page
{
    SessionCustom SC;
    DataAccess da;
    PhTemplate PH;
    ...
    {
        string str_client = "select ICL_username,ICL_userid from ICL_Users  where ICL_role=1  ";
        SqlCommand cmd = new SqlCommand(str_client);
        DataTable dt = da.GetDataTable(cmd);
        ddl_User.Items.Clear();
        ddl_User.Items.Insert(0, new ListItem("Select username ", "0"));
        if (dt.Rows.Count > 0)
        {
    ...
    protected void Btn_Submit_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            string usercheck = "select org_name,org_ccode,org_userid from ICl_Organization where org_userid='" +ddl_User.SelectedValue + "' and org_name='"+txt_orgname.Text+"'";
            ...
            if (dt_user.Rows.Count > 0)
            {
                ...
                org_userid = dt_user.Rows[0]["org_userid"].ToString();
            }


            if (Request.QueryString["id"] != null)
            {
                string updatequery = "UPDATE ICl_Organization SET org_name=@org_name,org_userid=@org_userid,org_ccode=org_ccode,org_address=@org_address,org_city=@org_city,org_country=@org_country,org_postcode=@org_country,org_billingname=@org_billingname,org_baddress=@org_baddress,org_bcity=@org_bcity,org_bcountry=@org_bcountry,org_bpostcode=@org_bpostcode,org_modifiedby=@org_modifiedby,org_modifiedon=@org_modifiedon WHERE org_id=@org_id";
                ...
                updatecmd.Parameters.AddWithValue("@org_modifiedby", this.str_userkey);
                ...
                Response.Redirect("~/Web/clientgrid.aspx");
            }
            ...
            else
            {
                if (dt_user.Rows.Count == 0)
                {
                    string sqlquery = "INSERT INTO ICl_Organization (org_name,org_ccode,org_userid,org_address,org_city,org_country,org_postcode,org_billingname,org_baddress,org_bcity,org_bcountry,org_bpostcode,org_createdby)" +
            "VALUES (@org_name,@org_ccode,@org_userid,@org_address,@org_city,@org_country,@org_postcode,@org_billingname,@org_baddress,@org_bcity,@org_bcountry,@org_bpostcode,@org_createdby)";
                    ...
                    cmd.Parameters.AddWithValue("@org_createdby", this.str_userkey);
                    da.ExecuteNonQuery(cmd);
                    string getuserid = "select ICL_Email,ICL_UserName,ICL_UserId from ICL_Users where ICL_UserId='" + ddl_User.SelectedValue + "'";
                    ...
                    if (dt_userid.Rows.Count > 0)
                    {
                        ...
                        string username = dt_userid.Rows[0]["ICL_UserName"].ToString();
                        string str_ukey = dt_userid.Rows[0]["ICL_UserId"].ToString();
                        string str_link = "http://206.206.125.70/createpassword.aspx?id=" + str_ukey + "";
                        string email_fun = this.CF.PasswordRecovery(str_mail, "Createpassword", "Create Password Recovery", str_link, username);
                    }
                ...
                {
                    lbl_error.Text = "This oragnisation assigned for this user is already exists";
