...
public partial class Web_Departing : System.Web.UI.Page
{
    DataAccess da;
    PhTemplate PH;
    Decrypt DE;
        ...
        try
        {
            ...
            using (SqlConnection conn = new SqlConnection(da.ConnectionString.ToString()))
            {
                ...
                if (searchValue == "")
                {
                    ...
                    string countQuery = "EXEC GetOrgETDCOUNT @Todaydate='" + dateOnly + "', @DD_status='" + Transport + "',@Clientcode='" + Clientcode + "',@controldata='" + controldata + "'";
                    SqlCommand countCmd = new SqlCommand(countQuery, conn);
                    object countResult = countCmd.ExecuteScalar();
                    totalRecords = countResult != null ? Convert.ToInt32(countResult) : 0;
                    string query = "EXEC GetOrgETDDEPARTINGDATA @Todaydate='" + dateOnly + "',@Start='" + start + "', @Length='" + length + "',@DD_status='" + Transport + "',@Clientcode='" + Clientcode + "'," + ascanddesc + ",@Pin='" + Pin + "'";
                ...
                {
                    // Get total record count
                    string countQuery = "EXEC GetOrgETDSEARCHCOUNT @Todaydate='" + dateOnly + "',@search='" + searchValue + "',@Clientcode='" + Clientcode + "',@controldata='" + controldata + "'";
                    SqlCommand countCmd = new SqlCommand(countQuery, conn);
                    object countResult = countCmd.ExecuteScalar();
                    totalRecords = countResult != null ? Convert.ToInt32(countResult) : 0;
                    string query = "EXEC GetOrgETDSEARCH @Todaydate='" + dateOnly + "',@search='" + searchValue + "',@Start='" + start + "',@Length='" + length + "',@Clientcode='" + Clientcode + "',@controldata='" + controldata + "'";
                    using (SqlCommand cmd = new SqlCommand(query, conn))
                    {
    ...
    public static string InsertShipment(string action, string shipmentid)
    {
        ...
        string shipmentcheckcheck = "select spr_shipmentid from ICL_PinnedDeparting where spr_shipmentid='" + shipmentid + "'";
        ...
        {
            action = "Inserted";
            query = "insert into ICL_PinnedDeparting(spr_shipmentid,ps_createdby,ClientCode)values(@spr_shipmentid,@ps_createdby,@ClientCode) ";
        }
        else
        {
            action = "Deleted";
            query = "DELETE FROM ICL_PinnedDeparting WHERE spr_shipmentid=@spr_shipmentid";
        }
        try
        {
