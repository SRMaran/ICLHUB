...
public partial class Web_Awaiting : System.Web.UI.Page
{
    DataAccess da;
    PhTemplate PH;
    Decrypt DE;
        ...
        try
        {
            ...
            using (SqlConnection conn = new SqlConnection(da.ConnectionString.ToString()))
            {
                ...
                if (searchValue == "")
                {
                    ...
                    string countQuery = "EXEC GetAwaitingdeliveryDATA @DD_status='" + Transport + "',@Clientcode='" + Clientcode + "',@controldata='" + controldata + "'";
                    SqlCommand countCmd = new SqlCommand(countQuery, conn);
                    object countResult = countCmd.ExecuteScalar();
                    totalRecords = countResult != null ? Convert.ToInt32(countResult) : 0;
                    string query = "EXEC GetAdminClientAwaitingdata @Start='" + start + "', @Length='" + length + "',@DD_status='" + Transport + "',@Clientcode='" + Clientcode + "'," + ascanddesc + ",@Pin='" + Pin + "'";
                ...
                {
                    // Get total record count
                    string countQuery = "EXEC GetAwaitingdeliverySearchcount @search='" + searchValue + "',@Clientcode='" + Clientcode + "',@controldata='" + controldata + "'";
                    SqlCommand countCmd = new SqlCommand(countQuery, conn);
                    object countResult = countCmd.ExecuteScalar();
                    totalRecords = countResult != null ? Convert.ToInt32(countResult) : 0;
                    string query = "EXEC GetAwaitingYeardataSearch @search='" + searchValue + "',@Start='" + start + "',@Length='" + length + "',@Clientcode='" + Clientcode + "',@controldata='" + controldata + "'";
                    using (SqlCommand cmd = new SqlCommand(query, conn))
                    {
    ...
    public static string InsertShipment(string action, string shipmentid)
    {
        ...
        string shipmentcheckcheck = "select spr_shipmentid from ICL_PinnedAwaiting where spr_shipmentid='" + shipmentid + "'";
        ...
        {
            action = "Inserted";
            query = "insert into ICL_PinnedAwaiting(spr_shipmentid,ps_createdby,ClientCode)values(@spr_shipmentid,@ps_createdby,@ClientCode) ";
        }
        else
        {
            action = "Deleted";
            query = "DELETE FROM ICL_PinnedAwaiting WHERE spr_shipmentid=@spr_shipmentid";
        }
        try
        {
